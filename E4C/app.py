...
# UPGRADED CSS: Added hover animations, glowing effects, and smooth transitions
st.markdown(f"""
    <style>
...
@st.dialog("Expanded Technical View", width="large")
def show_expanded_chart(plot_type, val, title, explanation):
    st.markdown(f"### {title}")
    if plot_type == "gauge":
        fig = go.Figure(go.Indicator(mode="gauge+number", value=val, 
                                     gauge={'axis': {'range': [0, 15]}, 'bar': {'color': PRIMARY_COLOR}}))
...
# ==========================================
# SIDEBAR CONTROLS (All sidebar logic stays here!)
# ==========================================
...
# We grab the manual mode outside the fragment so the sidebar doesn't break
if not auto_mode:
...
def render_live_dashboard():
    # Evaluate the mode without calling st.sidebar
    if not auto_mode:
        mode = manual_mode
    ...
    try:
        res = requests.post("http://127.0.0.1:8000/predict", json={"Irradiance":ir,"Current":cu,"Temp":te,"Vibration":vi,"Flow":fl,"Head":he}).json()
        ...
        st.error("Awaiting Backend Link... Please run the FastAPI server.")
...
# ==========================================
# UPGRADED ASSET TAB (Integrating E4C Docx Report)
# ==========================================
with tab_asset:
...
with tab_chat:
    ...
    if prompt := st.chat_input("E.g., What is the baseline current for the RainMaker 2?"):
        ...
        # Simple Logic rule-based AI using E4C context
